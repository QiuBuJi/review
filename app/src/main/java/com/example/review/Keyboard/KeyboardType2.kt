...
class KeyboardType2(context: Context, keyboardView: RecyclerView, container: ConstraintLayout, input: EditText, reviewStruct: ReviewStruct)
    ...
    lateinit var frameInput: ArrayList<WordExplain>
    lateinit var frameRight: ArrayList<WordExplain>
    lateinit var handleInterface: HandleInterfaceType2
    ...
    private fun wideLayout(): ArrayList<KeyText> {
        ...
        if (libTemp.isEmpty()) {
            //在库内取出类型为2且不是引用的数据
            val filter = MainActivity.data.library.filter { it.type == 2 && it.refer == 0 }
            libTemp.addAll(filter)
        }
        ...
        //去掉多余的数据
        for (i in 0 until size) if (wordsAdded.isNotEmpty()) wordsAdded.removeFirst()
    ...
    override fun keyDown(keyCode: Int, key: Char, posi: Int): Boolean {
        ...
        if (keyText.isCom) {
            ...
            if (isSuccess) {
                ...
                keyText.isPressed = (wordCountRight == wordCountInput || wordCountRight == 0)
    ...
    companion object {
        var libTemp: LibraryList = LibraryList()
    }
}